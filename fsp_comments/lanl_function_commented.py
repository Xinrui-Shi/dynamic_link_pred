...
def find_dimension(data):
    ...
    ### %% FSP: subtracting np.min(edge_array[:,0]) might cause 'undesired' effects 
    ###         if the labelling of the nodes is not 'nice' (i.e. sequential natural numbers)
...
def ef_hadamard(A,U,V,negative_class):
    ...
    ## Calculate the scores for negative_class
    scores_negative_class = []
    ### %% FSP: Same issue also here -- No need to add :n_edges.
    for pair in negative_class[:n_edges,:]:
